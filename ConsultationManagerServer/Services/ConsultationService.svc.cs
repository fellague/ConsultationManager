        ...
        public ServicePathologies GetServiceDetails()
        {
            ...
            else
            {
                try
                {
                    ...
                    if (db.Consultations.Exists())
                    {
                        var pathols = db.Consultations.AsQueryable().Where(p => p.IdService == servPathol.Service.Id).ToList();
                        //var pathols = db.Pathologies.FindAll().Where(p => p.IdService == service.Id).ToList();
                        if (pathols.Count() > 0)
                            foreach (Consultation item in pathols)
